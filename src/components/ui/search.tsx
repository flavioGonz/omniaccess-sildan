...
export function Seek({
  ...
  /* ── ADDED FOR OMNIACCESS ────────────────────────────────
     ...
     the value has to leave the component, the placeholder has
     to say what is being searched, and a filter that starts
     shut in a toolbar is a filter nobody finds. None of these
     change the object — they are the wiring it never needed
     while it sat alone on a card. */
  value: valorFuera,
  onChange,
  placeholder = "Search",
  startOpen = false,
  ...
  /* Controlled when a caller passes a value, its own otherwise.
     The original was always its own; a filter has to be able to
     be cleared from outside (the "limpiar filtros" button). */
  const gobernado = valorFuera !== undefined;
  const value = gobernado ? valorFuera : propio;
  const poner = (v: string) => { if (!gobernado) setPropio(v); onChange?.(v); };
  ...
  /* Away with nothing typed and it goes back. Away with
     ...
     throw it away. */
  const away = () => {
    if (value.trim() || startOpen) return;
    setOpen(false);
  };
      ...
      <div className="sek-skin">
        ...
        <input
          ...
          /* ── NO SOFTWARE KEYBOARD ON A TOUCH SCREEN ──────
             ...
             ── AND NOT HERE. ────────────────────────────────
             In OmniAccess this IS a search somebody is
             performing: a guard on a tablet looking up a plate
             needs the keyboard the original was suppressing.
             The reasoning above is kept because it is the
             reason this line has a condition at all. */
          tabIndex={open ? 0 : -1}
          onChange={(e) => { poner(e.target.value); tapped(); }}
          onBlur={away}
          onKeyDown={(e) => {
