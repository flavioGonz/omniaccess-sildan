...
const tabTransition = { type: "tween" as const, ease: [0.22, 1, 0.36, 1] as [number, number, number, number], duration: 0.16 };
...
export default function GuardConsole({ initialEntries, logo, headerColor, initialIcons, units, guards }: GuardConsoleProps) {
    ...
    const [recEvent, setRecEvent] = useState<any>(null);
    const [parkingByPlate, setParkingByPlate] = useState<Record<string, string>>({});
    ...
    // El Reporte de Inteligencia se abre A DEMANDA (tocando la matrícula), no automáticamente.
    // Al cerrar el visor limpiamos el análisis previo.
    useEffect(() => {
        if (!viewerData?.plate || viewerData.plate === '--- ---') setPlateAnalysis(null);
    }, [viewerData?.plate]);

    // Disparar el reporte a demanda
    const openPlateAnalysis = useCallback(async (plate?: string) => {
        const p = (plate || "").trim();
        if (!p || p === '--- ---') return;
        setLoadingAnalysis(true);
        try { const analysis = await getPlateAnalysis(p); setPlateAnalysis(analysis); }
        catch { }
    ...
    useEffect(() => {
        ...
        if (showCameraModal) {
            if (!window.isSecureContext || !navigator.mediaDevices?.getUserMedia) {
                showNotification("Cámara no disponible", "La cámara requiere HTTPS (o el flag de origen seguro en Chrome).", "error");
    ...
    useEffect(() => {
        ...
        newSocket.on('alert_status', (data: any) => {
            ...
            setIsAlertMode(prev => { previousState = prev; return data.active; });
            ...
            if (data.active) {
                ...
                showNotification("SISTEMA NORMALIZADO", message, "success", 5000);
    ...
    useEffect(() => {
        ...
        const request = async () => {
            ...
            } catch (e) { /* no soportado / sin permiso */ }
        };
        const onVis = () => {
            if (document.visibilityState === "visible") {
                request();
                setResyncTick((t) => t + 1); // resync al volver a foreground
            }
        };
        request();
    ...
    // Foto simple (sin detección LPR/OCR): solo guarda la imagen capturada.
    const handleSimplePhoto = (imageBlob: Blob) => {
    ...
    useEffect(() => {
        if (isAlertMode) {
            ...
            if (!alarmAudioRef.current) {
                alarmAudioRef.current = new Audio("/sounds/alarm_clock.ogg"); // alarma local (funciona sin internet)
    ...
    const handleTabChange = (tab: TabType) => {
        ...
        // Dirección de la transición según el orden del menú (feel nativo)
        const oi = TAB_ORDER.indexOf(activeTab);
        const ni = TAB_ORDER.indexOf(tab);
        if (oi >= 0 && ni >= 0) setTabDir(ni >= oi ? 1 : -1); else setTabDir(1);

        setActiveTab(tab);
        if (tab === "face") setFaceMatchResult(null);
    };

    // Swipe horizontal para cambiar de tab (dedo)
    ...
    const onContentTouchEnd = (e: React.TouchEvent) => {
        ...
        if (Math.abs(dx) < 70 || Math.abs(dx) < Math.abs(dy) * 1.6) return; // horizontal claro
    ...
    const handleManualLogin = async (e: React.FormEvent) => {
        ...
        const res = await verifyGuardCredential(loginUser, loginPass);
    ...
    useEffect(() => {
        ...
        return () => clearTimeout(timer);
    }, [historySearch, activeTab, resyncTick]);
    ...
    useEffect(() => {
        if (activeTab !== "lpr") return;
        // Merodeo (plates limpias de bitácora MERODEO)
        fetch("/api/history/merodeo")
            .then(r => r.json())
            .then(d => setMerodeoSet(new Set((d.plates || []).map((p: string) => (p || "").toUpperCase().replace(/[^A-Z0-9]/g, "")))))
            .catch(() => { });
        // Plaza asignada por matrícula (reverso de la ocupación de plazas)
        Promise.all([getParkingSlots(), getParkingOccupancy()]).then(([slots, occ]) => {
            const labelById: Record<string, string> = {};
            (slots as any[]).forEach(s => { labelById[s.id] = s.label; });
            const map: Record<string, string> = {};
            for (const [slotId, o] of Object.entries(occ as Record<string, any>)) {
                for (const plate of (o?.all || [])) {
                    map[(plate || "").toUpperCase().replace(/[^A-Z0-9]/g, "")] = labelById[slotId] || "";
                }
            }
            setParkingByPlate(map);
        }).catch(() => { });
    }, [activeTab, resyncTick]);
    ...
    const handleAnnexPanic = async () => {
        ...
        try {
            ...
            await createBitacoraEntry(formData);
            setNotes("");
    ...
    return (
        <div className="h-screen w-screen bg-white text-black flex overflow-hidden font-sans select-none">
                ...
                {isTransitioning && (
                    ...
                    >
                        ...
                        <motion.p
                            ...
                            className="mt-8 text-[11px] font-bold uppercase tracking-[0.4em] text-[#B20D30]"
                        >
                ...
                {showAlarmSplash && (
                    ...
                    >
                        ...
                        <motion.div
                            ...
                            className="w-48 h-48 rounded-2xl bg-white flex items-center justify-center text-[#B20D30] mb-10 shadow-[0_0_100px_rgba(255,255,255,0.4)] relative z-10"
                        >
                        ...
                        <div className="text-center relative z-10 space-y-4 px-10">
                            ...
                            <motion.p
                                ...
                                className="text-xl font-bold uppercase tracking-[0.4em] text-white/60"
                            >
                    ...
                    >
                        ...
                        <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-red-600 to-transparent flex items-center justify-center px-10">
                            <div className="flex items-center gap-6 text-white">
                                ...
                                <div className="text-center">
                                    <h2 className="text-4xl font-bold uppercase tracking-tighter">MODO ALERTA ACTIVO</h2>
                                    <p className="text-sm font-bold text-white/80 uppercase tracking-widest mt-1">El centro de monitoreo ha sido notificado</p>
                                </div>
                                <Siren size={48} className="animate-bounce" />
                            </div>
                        </div>
                        ...
                        >
                            <button
                                ...
                                className="w-56 h-56 rounded-full bg-white border-8 border-red-600 shadow-lg flex flex-col items-center justify-center gap-4 relative overflow-hidden group active:scale-95 transition-transform"
                            >
                                <motion.div
                                ...
                                <div className="text-center px-4 relative z-10">
                                    <p className="text-red-600 font-bold text-xs uppercase tracking-widest">Normalizar</p>
                                    <p className="text-red-600 font-bold text-[10px] uppercase opacity-40 mt-1">Mantener pulsado</p>
                                </div>
                            ...
                            <motion.div
                                ...
                                transition={{ repeat: Infinity, duration: 2 }}
                                className="px-6 py-2 bg-red-600 text-white rounded-full text-[10px] font-bold uppercase tracking-[0.3em] mb-4"
                            >
                            ...
                            {(capturedPhoto || audioUrl || notes) && (
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        className="px-8 py-3 bg-white text-emerald-600 rounded-2xl font-bold uppercase text-xs tracking-widest shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center gap-2"
                                    >
            ...
            <AnimatePresence>
                {showMatchPrompt && (
                    ...
                    >
                        ...
                        <div className="bg-white border-2 border-[#B20D30] rounded-2xl p-10 shadow-[0_40px_120px_rgba(0,0,0,0.15)] max-w-sm w-full relative z-10 pointer-events-auto overflow-hidden">
                            ...
                            <h3 className="text-2xl font-bold text-center uppercase tracking-tighter mb-4 relative z-10 text-black">¿Autocompletar?</h3>
                            <p className="text-black font-bold uppercase text-center text-[11px] font-bold uppercase tracking-widest leading-relaxed mb-10 relative z-10 px-4">
                                Se ha encontrado un registro previo para la matrícula <span className="text-[#B20D30] font-bold">{plate}</span>.
                            </p>

                            <div className="space-y-4 relative z-10">
                                <Button onClick={handleAutocomplete} className="w-full h-16 rounded-2xl bg-[#B20D30] hover:bg-[#910a28] text-white font-bold uppercase text-[12px] tracking-widest shadow-xl shadow-[#B20D30]/20">
                                    Sí, Cargar Datos
                                </Button>
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative pb-24">
                <div className="flex-1 overflow-hidden relative" onTouchStart={onContentTouchStart} onTouchEnd={onContentTouchEnd}>
                    <AnimatePresence mode="wait" custom={tabDir}>
                        {activeTab === "map" && (
                            <motion.div key="map" custom={tabDir} variants={tabVariants} initial="enter" animate="center" exit="exit" transition={tabTransition} className="h-full w-full relative z-0">
                                <GuardParkingView />
                            </motion.div>
                        )}
                        {activeTab === "guardmap" && (
                            <motion.div key="guardmap" custom={tabDir} variants={tabVariants} initial="enter" animate="center" exit="exit" transition={tabTransition} className="h-full w-full relative z-0">
                                <LiveGuardMap
                                    myLocation={myLocation}
                                    guards={otherGuards}
                                    socketId={socket?.id}
                                    onLongPress={(latlng: any) => { setBackupLocation(latlng); setShowBackupModal(true); playTactileSound(); }}
                                    backupMissions={monitoringMissions.map((m: any) => ({ ...m, responderLocation: m.responderId === socket?.id ? myLocation : (otherGuards.find((g: any) => g.socketId === m.responderId) || null) }))}
                                    onAlertClick={(mission: any) => { setActiveMission(mission); setShowResolutionModal(true); setBackupDetail(mission.details || ""); }}
                                />
                            </motion.div>
                        )}
                        {activeTab === "face" && (
                        ...
                        {activeTab === "control" && (
                            <motion.div key="control" custom={tabDir} variants={tabVariants} initial="enter" animate="center" exit="exit" transition={tabTransition} className="h-full w-full overflow-y-auto p-4 pr-20 md:p-8 pb-64 custom-scrollbar">
                                ...
                                <div className="max-w-7xl mx-auto flex flex-col gap-10">
                                    ...
                                    <div className="flex items-center gap-6">
                                        <motion.button
                                            ...
                                            className={cn(
                                                "flex-1 h-20 md:h-28 rounded-[1.5rem] md:rounded-2xl border-2 flex flex-col items-center justify-center gap-1 md:gap-2 transition-all relative overflow-hidden group",
                                                type === "ENTRY"
                                        ...
                                        >
                                            ...
                                            <span className="font-bold text-[9px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em]">Registro de Ingreso</span>
                                        ...
                                        >
                                            ...
                                            <span className="font-bold text-[9px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em]">Registro de Salida</span>
                                            ...
                                            <div className="bg-transparent space-y-10 group transition-all">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                    ...
                                                    <div className="col-span-1 md:col-span-2 space-y-3 pt-2 relative">
                                                        <Label className="text-[10px] font-bold uppercase text-black font-bold uppercase tracking-widest flex items-center gap-1.5 ml-1">
                                                        ...
                                                        <motion.button
                                                            ...
                                                            className={cn(
                                                                "w-full h-16 md:h-24 rounded-[1.5rem] md:rounded-2xl border border-black/20 flex items-center justify-between px-6 md:px-8 transition-all relative overflow-hidden group",
                                                                selectedUnit
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-4 md:gap-6">
                                                                ...
                                                                <div className="text-left">
                                                                    {selectedUnit ? (
                                                                        <>
                                                                            <p className="text-base md:text-xl font-bold text-black uppercase tracking-tight">{selectedUnit.name}</p>
                                                                            <p className="text-[9px] md:text-[10px] font-bold text-[#B20D30] uppercase tracking-widest">{selectedUnit.number || "LT"}</p>
                                                                        </>
                                                                    ) : (
                                                                        <>
                                                                            <p className="text-sm md:text-lg font-bold text-black/30 uppercase tracking-tighter">Pendiente Seleccionar</p>
                                                                            <p className="text-[8px] md:text-[9px] font-bold text-black/20 uppercase tracking-widest">Toca para abrir el panel de unidades</p>
                                                                        </>
                                                                    )}
                                                                </div>
                                                            </div>
                                                    ...
                                                    <div className="col-span-1 md:col-span-2 space-y-4">
                                                        <Label className="text-[11px] font-bold uppercase text-black font-bold uppercase tracking-[0.2em] ml-2">Observaciones</Label>
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        {audioUrl && (
                                                            <div className="flex items-center gap-3 bg-emerald-50 border-2 border-emerald-500/20 px-4 py-3 rounded-2xl shadow-sm">
                                                                ...
                                                                <div className="text-[9px] font-bold text-emerald-700 uppercase tracking-widest">Audio Grabado</div>
                                                        ...
                                                        {!capturedPhoto && !audioUrl && (
                                                            <div className="flex items-center gap-3 bg-slate-50 border border-dashed border-black/20 transition-all duration-300 px-6 py-4 rounded-2xl">
                                                                ...
                                                                <span className="text-[9px] font-bold text-black/20 uppercase tracking-[0.2em]">Sin Adjuntos Multimedia</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                ...
                                {/* FOTO SIMPLE (sin detector LPR por cámara) */}
                                <AnimatePresence>
                                    {isOCRActive && (
                                        <SimplePhotoCapture
                                            onCapture={handleSimplePhoto}
                                            ...
                                            <div className="w-full max-w-7xl h-full flex flex-col gap-10">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-8">
                                                        <div className="w-20 h-20 rounded-2xl bg-[#B20D30] flex items-center justify-center text-white shadow-[0_20px_60px_rgba(178,13,48,0.4)]">
                                                            <Home size={40} />
                                                        </div>
                                                        <div>
                                                            <h2 className="text-5xl font-bold text-white uppercase tracking-tighter">Unidades y Lotes</h2>
                                                            <p className="text-sm font-bold text-white/50 uppercase tracking-[0.4em] mt-3">Seleccione el punto de destino final</p>
                                                        </div>
                                                    </div>
                                                    <button
                                                        onClick={() => setShowUnitPicker(false)}
                                                        className="w-20 h-20 bg-white hover:bg-[#B20D30] rounded-2xl flex items-center justify-center text-black hover:text-white transition-all duration-500 border-2 border-white group"
                                                    >
                                                ...
                                                <div className="flex items-center gap-10">
                                                    ...
                                                    <Input
                                                        ...
                                                        className="flex-1 h-20 bg-white/5 border-2 border-white/10 rounded-2xl text-3xl font-bold text-white placeholder:text-white/20 px-10 focus:border-[#B20D30] transition-all"
                                                    />
                                                </div>
                                                ...
                                                <div className="flex-1 overflow-y-auto pr-6 custom-scrollbar grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-8 pb-10">
                                                    {units.filter(u => u.name.toLowerCase().includes(unitSearch.toLowerCase()) || (u.number && u.number.toLowerCase().includes(unitSearch.toLowerCase()))).map((u) => (
                                                        <motion.button
                                                            ...
                                                            className={cn(
                                                                "min-h-[220px] rounded-2xl border-2 flex flex-col items-center justify-center gap-6 transition-all relative overflow-hidden group",
                                                        ...
                                                        >
                                                            ...
                                                            <div className="text-center px-6">
                                                                <p className={cn(
                                                                    "text-2xl font-bold uppercase tracking-tighter leading-none break-words line-clamp-2 transition-colors",
                                                                    selectedUnit?.id === u.id ? "text-black" : "text-white"
                                                                )}>{u.name}</p>
                                                                <p className={cn(
                                                                    "text-xs font-bold uppercase tracking-widest mt-4 transition-colors",
                                            ...
                                            <div className="w-full max-w-5xl flex flex-col gap-10">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-8">
                                                        ...
                                                        <div>
                                                            <h2 className="text-5xl font-bold text-white uppercase tracking-tighter">Clasificación de Origen</h2>
                                                            <p className="text-sm font-bold text-white/50 uppercase tracking-[0.4em] mt-3">Indique el tipo de visitante o entidad</p>
                                                        </div>
                                                    </div>
                                                    <button
                                                        onClick={() => setShowOriginPicker(false)}
                                                        className="w-20 h-20 bg-white hover:bg-[#B20D30] rounded-2xl flex items-center justify-center text-black hover:text-white transition-all duration-500 border-2 border-white"
                                                    >
                                                        <X size={40} />
                                                    </button>
                                                </div>
                                ...
                                <div className="fixed top-1/2 -translate-y-1/2 right-2 md:right-6 z-[90]">
                                    <div className={cn(
                                        "flex flex-col gap-2 md:gap-3 p-2 md:p-3 rounded-2xl md:rounded-2xl border shadow-[0_10px_60px_-10px_rgba(0,0,0,0.15)] ring-1 ring-black/5 transition-all duration-500",
                                        isAlertMode
                                        ...
                                        >
                                            {selectedUnit ? (
                                                <span className="text-sm md:text-lg font-bold">{selectedUnit.number || selectedUnit.name.substring(0, 2)}</span>
                                            ) : (
                                        ...
                                        >
                                            ...
                                            <span className="text-[6px] md:text-[7px] font-bold uppercase tracking-tighter">Auto</span>
                                        </motion.button>
                                        ...
                                        >
                                            ...
                                            <span className="text-[6px] md:text-[7px] font-bold uppercase tracking-tighter">Moto</span>
                                        </motion.button>
                        ...
                        {activeTab === "alerts" && (
                            <motion.div key="alerts" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="h-full w-full overflow-y-auto px-8 pt-6 pb-40 custom-scrollbar">
                                <div className="max-w-7xl mx-auto space-y-6">
                                    {/* HEADER + FILTER ICONS */}
                                    <div className="flex items-center justify-between sticky top-0 bg-white/50 backdrop-blur-2xl rounded-2xl border border-white/60 px-4 py-4 z-30 shadow-sm">
                                        <div>
                                            <h2 className="text-3xl font-bold uppercase tracking-tighter text-black leading-none">Eventos Críticos</h2>
                                            <p className="text-[10px] text-[#B20D30] font-bold uppercase tracking-[0.3em] mt-2">Registro de activación de botón de pánico</p>
                                        </div>
                                    ...
                                    <AnimatePresence>
                                        {showAlertsFilterModal && (
                                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setShowAlertsFilterModal(false)}>
                                                <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white rounded-2xl p-8 w-full max-w-md shadow-lg space-y-6" onClick={e => e.stopPropagation()}>
                                                    <div className="flex items-center justify-between">
                                                        <h3 className="text-xl font-bold uppercase tracking-tighter">Filtros</h3>
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="space-y-2">
                                                            <label className="text-[10px] font-bold uppercase text-black/40 tracking-widest">Fecha</label>
                                                            <Input type="date" value={alertsDate} onChange={(e) => setAlertsDate(e.target.value)} className="h-14 bg-slate-50 border-slate-200 rounded-2xl font-bold text-sm" />
                                                        </div>
                                                        <div className="space-y-2">
                                                            <label className="text-[10px] font-bold uppercase text-black/40 tracking-widest">Operario</label>
                                                            <Input placeholder="Buscar operario..." value={alertsSearch} onChange={(e) => setAlertsSearch(e.target.value)} className="h-14 bg-slate-50 border-slate-200 rounded-2xl font-bold text-sm" />
                                                        </div>
                                                    </div>
                                                    <Button onClick={() => setShowAlertsFilterModal(false)} className="w-full h-14 rounded-2xl bg-[#B20D30] hover:bg-[#910a28] text-white font-bold uppercase tracking-widest">Aplicar</Button>
                                                </motion.div>
                                            </motion.div>
                                        )}
                                    </AnimatePresence>
                                        ...
                                        {(() => {
                                            ...
                                            if (alertArr.length === 0) return (
                                                <div className="py-32 flex flex-col items-center gap-6 opacity-20 text-black">
                                                    <Shield size={80} />
                                                    <p className="text-xl font-bold uppercase tracking-[0.5em]">Historial de Alertas Limpio</p>
                                                </div>
                                            );
                                                ...
                                                return (
                                                    <motion.button key={alert.id} whileTap={{ scale: 0.98 }} onClick={() => setSelectedEntry(alert)}
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-sm font-bold uppercase text-black truncate">{alert.guardName || "Sistema"}</p>
                                                            <p className={cn("text-[10px] font-bold uppercase tracking-wider", isSOS ? "text-orange-600" : "text-red-600")}>{isSOS ? "Informó Sospechoso" : isStillActive ? "Alerta Activa" : "Alerta Resuelta"}</p>
                                                        </div>
                        ...
                        {activeTab === "lpr" && (
                            <motion.div key="lpr" custom={tabDir} variants={tabVariants} initial="enter" animate="center" exit="exit" transition={tabTransition} className="h-full w-full overflow-y-auto px-8 pt-6 pb-40 custom-scrollbar">
                                <div className="max-w-7xl mx-auto space-y-6">
                                    <div className="flex items-center justify-between sticky top-0 bg-white/50 backdrop-blur-2xl rounded-2xl border border-white/60 px-4 py-4 z-30 shadow-sm">
                                        <div>
                                            <h2 className="text-3xl font-bold uppercase tracking-tighter text-black leading-none">Historial LPR</h2>
                                            <p className="text-[10px] text-[#B20D30] font-bold uppercase tracking-[0.3em] mt-2">Reconocimiento automático de matrículas</p>
                                        </div>
                                        <div className="flex items-center gap-2">
                                    ...
                                    <AnimatePresence>
                                        {showLprFilterModal && (
                                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setShowLprFilterModal(false)}>
                                                <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white rounded-2xl p-8 w-full max-w-md shadow-lg space-y-6" onClick={e => e.stopPropagation()}>
                                                    <div className="flex items-center justify-between">
                                                        <h3 className="text-xl font-bold uppercase tracking-tighter">Filtros LPR</h3>
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="space-y-2"><label className="text-[10px] font-bold uppercase text-black/40 tracking-widest">Fecha</label><Input type="date" value={lprDate} onChange={(e) => setLprDate(e.target.value)} className="h-14 bg-slate-50 border-slate-200 rounded-2xl font-bold text-sm" /></div>
                                                        <div className="space-y-2"><label className="text-[10px] font-bold uppercase text-black/40 tracking-widest">Matrícula</label><Input placeholder="Buscar matrícula..." value={lprSearch} onChange={(e) => setLprSearch(e.target.value)} className="h-14 bg-slate-50 border-slate-200 rounded-2xl font-bold text-sm" /></div>
                                                        <div className="space-y-2">
                                                            <label className="text-[10px] font-bold uppercase text-black/40 tracking-widest">Dirección</label>
                                                            <div className="flex gap-2">
                                                                {(["ALL", "ENTRY", "EXIT"] as const).map(dir => (
                                                                    <button key={dir} onClick={() => setLprDirection(dir)} className={cn("flex-1 h-14 rounded-2xl font-bold text-sm uppercase tracking-wider transition-all border", lprDirection === dir ? dir === "ENTRY" ? "bg-emerald-500 text-white border-emerald-500" : dir === "EXIT" ? "bg-orange-500 text-white border-orange-500" : "bg-[#B20D30] text-white border-[#B20D30]" : "bg-slate-50 text-black/40 border-slate-200 hover:border-slate-300")}>
                                    ...
                                    <div className="space-y-4">
                                        {isLprLoading ? (
                                            ...
                                            lprEntries.map((event) => {
                                                ...
                                                const plaza = parkingByPlate[cleanP];
                                                ...
                                                return (
                                                  <div key={event.id} className="flex items-stretch gap-2">
                                                    <motion.button whileTap={{ scale: 0.98 }} onClick={() => setViewerData({ url: event.snapshotPath || "", plate: event.plateDetected, name: event.user?.name, unit: event.user?.unit?.name, direction: event.direction, confidence: event.confidence, timestamp: event.timestamp, deviceName: event.device?.name, vehicleBrand: vehicle?.brand, vehicleModel: vehicle?.model })} className={cn("flex-1 min-w-0 rounded-[1.5rem] border p-5 flex items-center gap-5 hover:shadow-xl transition-all text-left group active:scale-[0.99] backdrop-blur-xl", isMerodeo ? "bg-red-50/70 border-red-200/70 shadow-red-500/5" : "bg-white/70 border-white/80 shadow-sm")}>
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                                <span className="px-2 py-0.5 bg-slate-100 border border-slate-200 rounded-md text-sm font-bold text-black uppercase tracking-tighter">{event.plateDetected || "--- ---"}</span>
                                                                <span className={cn("px-2 py-0.5 rounded-md text-[9px] font-bold uppercase", event.direction === "ENTRY" ? "bg-emerald-50 text-emerald-600" : "bg-orange-50 text-orange-600")}>{event.direction === "ENTRY" ? "Ingreso" : "Egreso"}</span>
                                                                {isMerodeo && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-bold uppercase bg-red-500/10 text-red-600 border border-red-500/20"><ShieldAlert size={11} /> Merodeo</span>}
                                                            </div>
                                                            ...
                                                            {/* Indicadores de la captura */}
                                                            <div className="flex items-center gap-1.5 mt-2">
                                                                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-indigo-500/10 text-indigo-600 text-[8px] font-bold uppercase tracking-wider" title="Ver análisis de la matrícula"><ScanSearch size={11} /> Análisis</span>
                                                                {plaza && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-emerald-500/10 text-emerald-600 text-[8px] font-bold uppercase tracking-wider" title={`Plaza asignada: ${plaza}`}><SquareParking size={11} /> {plaza}</span>}
                                                                {isUnknown && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-amber-500/10 text-amber-600 text-[8px] font-bold uppercase tracking-wider" title="Registrar nuevo usuario"><UserPlus size={11} /> Registrar</span>}
                                                                {isMerodeo && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-red-500/10 text-red-600 text-[8px] font-bold uppercase tracking-wider" title="Vehículo con merodeo detectado"><ShieldAlert size={11} /> Merodeo</span>}
                                                            </div>
                                                        </div>
                                                        <div className="text-right shrink-0">
                                                    ...
                                                    <button onClick={() => { setRecEvent(event); playTactileSound(); }} title="Ver grabación" className="shrink-0 w-16 rounded-[1.25rem] bg-slate-900 text-white flex flex-col items-center justify-center gap-1 active:scale-95 hover:bg-black transition-all">
                                                        <Video size={20} /><span className="text-[8px] font-bold uppercase tracking-widest">Grab.</span>
                                                    </button>
                                                  </div>
                                                );
                                            })
                                        ) : (
                                            <div className="py-32 flex flex-col items-center gap-6 opacity-20"><Search size={80} /><p className="text-xl font-bold uppercase tracking-[0.5em]">No se encontraron registros</p></div>
                                        )}
                                    </div>
                                </div>
                            </motion.div>
                        ...
                        {activeTab === "history" && (
                            <motion.div key="history" custom={tabDir} variants={tabVariants} initial="enter" animate="center" exit="exit" transition={tabTransition} className="h-full w-full overflow-y-auto px-8 pt-6 pb-40 custom-scrollbar">
                                <div className="max-w-7xl mx-auto space-y-8">
                                    <div className="flex flex-col md:flex-row md:items-center justify-between sticky top-0 bg-slate-50/95 backdrop-blur-md py-6 z-30 gap-6">
                                        <div>
                                            <h2 className="text-3xl font-bold uppercase tracking-tighter text-black leading-none">Bitácora Digital</h2>
                                            <p className="text-[10px] text-black/40 font-bold uppercase tracking-[0.3em] mt-2">Registros de ingreso y salida en tiempo real</p>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            <div className="relative">
                                                ...
                                                <Input
                                                    ...
                                                    className="pl-12 h-14 w-80 bg-white/40 backdrop-blur-md border border-black transition-all duration-300 rounded-2xl font-bold text-xs uppercase tracking-widest transition-all focus:border-[#B20D30]/50 shadow-sm"
                                                    value={historySearch}
                                                    onChange={(e) => {
                                    ...
                                    <div className="w-full">
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                            {entries.map((entry) => (
                                                <motion.button
                                                    ...
                                                    className="bg-white rounded-2xl border-2 border-slate-100 p-6 flex flex-col gap-5 hover:shadow-xl transition-all text-left group relative overflow-hidden"
                                                >
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <div className="flex items-center gap-2 mb-1">
                                                                    <span className="px-3 py-1 bg-black text-white rounded-lg text-lg font-bold uppercase tracking-widest font-mono shadow-lg shadow-black/20">
                                                                        {entry.plate || "S/N"}
                                                                    </span>
                                                                    {entry.type === "ALERTA" ? (
                                                                        <Badge className="bg-red-600 text-white font-bold animate-pulse border-none text-[10px]">ALERTA</Badge>
                                                                    ) : (
                                                                        <Badge className={cn("font-bold text-[10px] border-none", entry.type === "ENTRY" ? "bg-indigo-600 text-white" : "bg-orange-600 text-white")}>
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="grid grid-cols-2 gap-4">
                                                            <div className="bg-slate-50 rounded-2xl p-3 border border-slate-100">
                                                                <p className="text-[10px] font-bold text-black/30 uppercase tracking-widest mb-1 flex items-center gap-1.5"><UserIcon size={12} /> Sujeto</p>
                                                                <p className="text-xs font-bold text-black truncate uppercase">{entry.name || "Reservado"}</p>
                                                                <p className="text-[10px] font-bold text-black/40 uppercase">{entry.dni || "N/A"}</p>
                                                            </div>
                                                            <div className="bg-slate-50 rounded-2xl p-3 border border-slate-100">
                                                                <p className="text-[10px] font-bold text-black/30 uppercase tracking-widest mb-1 flex items-center gap-1.5"><Home size={12} /> Destino</p>
                                                                <p className="text-xs font-bold text-[#B20D30] truncate uppercase">{entry.destination || "---"}</p>
                                                            </div>
                                                        </div>

                                                        <div className="bg-slate-50 rounded-2xl p-3 border border-slate-100 flex items-center justify-between">
                                                            <div className="flex flex-col">
                                                                <p className="text-[10px] font-bold text-black/30 uppercase tracking-widest mb-1 flex items-center gap-1.5"><Camera size={12} /> Cámara / Origen</p>
                                                    ...
                                                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-50">
                                                        <span className="text-[8px] font-bold text-black/20 uppercase tracking-[0.3em]">Operador: {entry.guardName || "---"}</span>
                                                        <div className="flex items-center gap-2">
                                                            <button
                                        ...
                                        <div ref={loadMoreRef} className="py-20 flex justify-center w-full">
                                            ...
                                            {!hasMoreHistory && !isHistoryLoading && (
                                                <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-black/20">Fin de la bitácora</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
                ...
                <AnimatePresence>
                    {selectedEntry && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] bg-black/60 backdrop-blur-sm" onClick={() => setSelectedEntry(null)}>
                            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }} className="absolute right-0 top-0 bottom-0 w-full max-w-lg bg-white shadow-lg overflow-y-auto" onClick={e => e.stopPropagation()}>
                                {/* Header */}
                                ...
                                <div className="p-6 space-y-6">
                                        ...
                                        {(() => {
                                            ...
                                            if (isPanic) return (
                                                <div className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-xl shadow-lg shadow-red-600/20 animate-pulse">
                                                    <Siren size={20} />
                                                    <span className="text-xl font-bold uppercase tracking-tighter">Pánico Crítico</span>
                                                </div>
                                            );
                                            if (isSos) return (
                                                <div className="flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-xl shadow-lg shadow-orange-500/20">
                                                    <ShieldAlert size={20} />
                                                    <span className="text-xl font-bold uppercase tracking-tighter">Reporte S.O.S</span>
                                                </div>
                                            );
                                            if (isDeactivation) return (
                                                <div className="flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-500 border-2 border-slate-200 rounded-xl">
                                                    ...
                                                    <span className="text-xl font-bold uppercase tracking-tighter">Manual</span>
                                                </div>
                                            );
                                                ...
                                                <>
                                                    {selectedEntry.plate && (
                                                        <div className="px-4 py-2 bg-slate-100 border-2 border-slate-200 rounded-xl">
                                                            <span className="text-2xl font-bold text-black uppercase tracking-tighter">{selectedEntry.plate}</span>
                                                        </div>
                                                    )}
                                                    <div className={cn(
                                                        "px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5",
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        {selectedEntry.name && (
                                            <div className="bg-slate-50 rounded-2xl p-4">
                                                <p className="text-[9px] font-bold uppercase text-black/30 tracking-widest mb-1">Nombre</p>
                                                <p className="text-sm font-bold text-black uppercase">{selectedEntry.name}</p>
                                            </div>
                                        )}
                                        {selectedEntry.dni && (
                                            <div className="bg-slate-50 rounded-2xl p-4">
                                                <p className="text-[9px] font-bold uppercase text-black/30 tracking-widest mb-1">Documento</p>
                                                <p className="text-sm font-bold text-black uppercase">{selectedEntry.dni}</p>
                                            </div>
                                        )}
                                        {selectedEntry.destination && (
                                            <div className="bg-slate-50 rounded-2xl p-4">
                                                <p className="text-[9px] font-bold uppercase text-black/30 tracking-widest mb-1">Destino</p>
                                                <p className="text-sm font-bold text-[#B20D30] uppercase">{selectedEntry.destination}</p>
                                            </div>
                                        )}
                                        {selectedEntry.company && (
                                            <div className="bg-slate-50 rounded-2xl p-4">
                                                <p className="text-[9px] font-bold uppercase text-black/30 tracking-widest mb-1">Empresa</p>
                                                <p className="text-sm font-bold text-black uppercase">{selectedEntry.company}</p>
                                            </div>
                                        )}
                                        {selectedEntry.guardName && (
                                            <div className="bg-slate-50 rounded-2xl p-4">
                                                <p className="text-[9px] font-bold uppercase text-black/30 tracking-widest mb-1">Guardia</p>
                                                <p className="text-sm font-bold text-black uppercase">{selectedEntry.guardName}</p>
                                            </div>
                                        )}
                                        {selectedEntry.origin && (
                                            <div className="bg-slate-50 rounded-2xl p-4">
                                                <p className="text-[9px] font-bold uppercase text-black/30 tracking-widest mb-1">Origen</p>
                                                <p className="text-sm font-bold text-black uppercase">{selectedEntry.origin}</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {selectedEntry.observations && (
                                        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
                                            <p className="text-[9px] font-bold uppercase text-amber-600 tracking-widest mb-2">Observaciones</p>
                                    ...
                                    {selectedEntry.audioPath && (
                                        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5">
                                            <p className="text-[9px] font-bold uppercase text-blue-600 tracking-widest mb-3">Audio Grabado</p>
                                            <audio controls src={selectedEntry.audioPath} className="w-full" />
                                        </div>
                                    )}

                                    {/* Delete */}
                                    <button onClick={() => { handleDelete(selectedEntry.id); setSelectedEntry(null); }} className="w-full py-4 rounded-2xl bg-red-50 hover:bg-red-100 text-[#B20D30] font-bold uppercase text-xs tracking-widest transition-colors flex items-center justify-center gap-2">
                                        <Trash2 size={16} /> Eliminar Registro
                                    </button>
                                </div>
                            </motion.div>
                ...
                <footer className={cn(
                    "fixed bottom-0 left-0 right-0 h-20 md:h-24 border-t transition-all duration-500 flex items-center justify-center px-4 md:px-6 z-[100] shadow-[0_-20px_60px_rgba(0,0,0,0.12)]",
                    isAlertMode ? "bg-red-600/90 backdrop-blur-2xl border-white/40" : "bg-white/60 backdrop-blur-2xl border-white/60"
                )}>
                    ...
                    <div className="absolute left-4 md:left-6 z-20">
                        ...
                        {/* R2: punto de conexión siempre visible */}
                        <span className={cn("absolute -top-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-white z-30 pointer-events-none", isConnected ? "bg-emerald-500" : "bg-red-500 animate-pulse")} title={isConnected ? "En línea" : "Sin conexión"} />
                                ...
                                <>
                                    ...
                                    >
                                        <div className="p-3 bg-slate-50 rounded-2xl mb-2">
                                            ...
                                            <p className="text-sm font-bold uppercase text-black truncate">{guardName || "Guardia"}</p>
                                        ...
                                        <button className="flex items-center gap-3 p-4 hover:bg-slate-50 rounded-2xl transition-colors text-black"
                                            ...
                                            <Camera size={20} /> <span className="text-xs font-bold uppercase tracking-wider">Mi Foto</span>
                                        </button>
                                        <button className="flex items-center gap-3 p-4 hover:bg-slate-50 rounded-2xl transition-colors text-black"
                                            onClick={() => { setShowSettingsModal(true); setShowProfileMenu(false); }}>
                                            <Settings size={20} /> <span className="text-xs font-bold uppercase tracking-wider">Configuración</span>
                                        </button>
                                        ...
                                        <button className="flex items-center gap-3 p-4 hover:bg-red-50 text-[#B20D30] rounded-2xl transition-colors"
                                            ...
                                            <LogOut size={20} /> <span className="text-xs font-bold uppercase tracking-wider">Cerrar Sesión</span>
                                        </button>
                                    </motion.div>
                                </>
                            )}
                        ...
                        <BottomTab icon={customIcons.history ? <Image src={customIcons.history} width={20} height={20} className="object-contain md:w-6 md:h-6" alt="Icon" /> : <HistoryIcon size={20} className="md:w-6 md:h-6" />} active={activeTab === "history"} onClick={() => handleTabChange("history")} label="Historial" alertActive={isAlertMode} />
                                ...
                                <div>
                                    <button
                                        ...
                                        onClick={() => { if (!isAlertMode) handleTabChange("alerts"); playTactileSound(); }}
                                        onContextMenu={(e) => e.preventDefault()}
                                    className={cn(
                                        "w-20 h-20 md:w-24 md:h-24 rounded-full shadow-lg flex items-center justify-center transition-all border-4 select-none relative overflow-hidden touch-none",
                                ...
                                >
                                    {/* Anillo de progreso del hold */}
                                    {panicHoldProgress > 0 && (
                                        <span className="absolute inset-0 rounded-full pointer-events-none" style={{ background: `conic-gradient(${isAlertMode ? "#fff" : "#B20D30"} ${panicHoldProgress * 3.6}deg, transparent 0deg)`, WebkitMask: "radial-gradient(circle, transparent 60%, #000 62%)", mask: "radial-gradient(circle, transparent 60%, #000 62%)" }} />
                                    )}
                                    <Siren size={32} className={cn("md:w-8 md:h-8 relative z-10")} />
                                </button>
                                {!isAlertMode && (
                                    <div className="absolute -top-9 left-1/2 -translate-x-1/2 text-[10px] font-bold uppercase text-red-600 whitespace-nowrap pointer-events-none opacity-50">
                                        Mantené presionado
                                    </div>
                                )}
                            </div>
                ...
                {/* CAPA NATIVA: sensores, man-down, dead-man, night, torch, geocerca, rondas, telemetría */}
                <GuardNativeLayer socket={socket} guardName={guardName} myLocation={myLocation} isAlertMode={isAlertMode} onPanic={toggleAlertMode} />
                    ...
                    {showResolutionModal && activeMission && (
                        <div className="fixed inset-0 z-[500] bg-black/80 flex items-center justify-center p-6">
                            <motion.div
                                ...
                                className="bg-white w-full max-w-md rounded-3xl p-6 shadow-lg text-center space-y-4"
                            >
                                <h2 className="text-2xl font-bold uppercase text-black">Gestionar Incidente</h2>
                                <p className="text-sm text-gray-500 uppercase font-bold">Seleccione una acción para finalizar</p>

                                <div className="space-y-2 text-left">
                                    <label className="text-[10px] font-bold uppercase text-gray-400 ml-2">Notas / Observaciones</label>
                                    <textarea
                                        value={backupDetail}
                                ...
                                <div className="grid grid-cols-1 gap-3 mt-4">
                                    <button onClick={() => {
                                        ...
                                        setBackupDetail("");
                                    }} className="bg-emerald-600 hover:bg-emerald-700 text-white py-5 rounded-2xl font-bold uppercase flex items-center justify-center gap-3 transition-all shadow-xl shadow-emerald-600/20 active:scale-95">
                                    ...
                                    <button onClick={() => {
                                        ...
                                        setShowResolutionModal(false);
                                        setBackupDetail("");
                                    }} className="bg-amber-500 hover:bg-amber-600 text-white py-5 rounded-2xl font-bold uppercase flex items-center justify-center gap-3 transition-all shadow-xl shadow-amber-500/20 active:scale-95">
                                    ...
                                    <button onClick={() => {
                                        ...
                                        setShowResolutionModal(false);
                                        setBackupDetail("");
                                    }} className="bg-gray-100 hover:bg-red-100 text-gray-500 hover:text-red-600 p-4 rounded-xl font-bold uppercase flex items-center justify-center gap-2 transition-colors">
                        ...
                        >
                            <div className="absolute top-8 left-1/2 -translate-x-1/2 z-10">
                                <div className="bg-white/10 backdrop-blur-xl border border-white/20 px-8 py-4 rounded-3xl flex items-center gap-4">
                                    ...
                                    <span className="text-white font-bold text-xl uppercase tracking-widest">Mi Foto de Perfil</span>
                                </div>
                            </div>
                            ...
                            <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex gap-6 z-10">
                                <motion.button
                                    ...
                                    className="w-24 h-24 rounded-full bg-white border-8 border-white/30 shadow-lg flex items-center justify-center hover:scale-110 transition-transform"
                                >
                                ...
                                <motion.button
                                    ...
                                    className="w-20 h-20 rounded-full bg-red-600 border-4 border-white/30 shadow-lg flex items-center justify-center hover:bg-red-700 transition-colors"
                                >
                    ...
                    {showBackupModal && (
                        <div className="fixed inset-0 z-[300] bg-black/80 flex items-center justify-center p-6 backdrop-blur-sm">
                            <motion.div
                                ...
                                animate={{ scale: 1, opacity: 1 }}
                                exit={{ scale: 0.9, opacity: 0 }}
                                className="bg-white w-full max-w-lg rounded-2xl p-8 shadow-lg relative overflow-hidden"
                            >
                                <div className="text-center mb-6 relative z-10">
                                    <h2 className="text-3xl font-bold uppercase text-[#B20D30] tracking-tighter">Reportar Incidente</h2>
                                ...
                                <div className="mb-6 relative z-10">
                                    <label className="text-[10px] uppercase font-bold text-gray-400 mb-2 block tracking-widest">Detalles Adicionales (Opcional)</label>
                                    <input
                                        type="text"
                                ...
                                <div className="grid grid-cols-2 gap-4 relative z-10">
                                    <button onClick={() => {
                                        ...
                                        <span className="text-sm font-bold uppercase text-slate-700 leading-tight">Individuo<br />Sospechoso</span>
                                    </button>
                                    ...
                                    <button onClick={() => {
                                        ...
                                        <span className="text-sm font-bold uppercase text-slate-700 leading-tight">Vehículo<br />Sospechoso</span>
                                    </button>
                                    ...
                                    <button onClick={() => {
                                        ...
                                        <span className="text-sm font-bold uppercase text-red-600 leading-tight">Solicitar Apoyo Necesario</span>
                                    </button>
                                </div>
                        ...
                        incomingBackup && (
                            <div className="fixed inset-0 z-[400] bg-red-600/90 flex flex-col items-center justify-center p-8">
                                <motion.div
                                    initial={{ scale: 0.9, opacity: 0 }}
                                    animate={{ scale: 1, opacity: 1 }}
                                    className="bg-white w-full max-w-lg rounded-3xl p-8 shadow-lg text-center space-y-6 border-4 border-red-500"
                                >
                                    ...
                                    <div>
                                        <h1 className="text-3xl font-bold uppercase text-red-600 leading-none mb-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mt-8">
                                        <button
                                            ...
                                            className="py-4 rounded-2xl border-2 border-gray-200 text-gray-500 font-bold uppercase tracking-widest hover:bg-gray-50"
                                        >
                                            Ignorar
                                        </button>
                                        <button
                                            ...
                                            className={cn(
                                                "py-4 rounded-2xl bg-red-600 text-white font-bold uppercase tracking-widest hover:bg-red-700 shadow-xl shadow-red-500/30",
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-4 bg-red-600/20 px-8 py-3 rounded-2xl border border-red-500/30">
                                        ...
                                        />
                                        <span className="text-white font-bold text-xs uppercase tracking-[0.4em]">REC ON AIR</span>
                                        <div className="h-4 w-px bg-white/10 mx-2" />
                                        <span className="text-white font-bold text-xl tabular-nums">{formatDuration(recordingDuration)}</span>
                                    </div>
                                    ...
                                    <div className="text-center">
                                        <h3 className="text-4xl font-bold text-white uppercase tracking-tighter mb-4">Capturando Nota de Audio</h3>
                                        <p className="text-white/40 font-bold text-[10px] uppercase tracking-[0.5em]">La grabación se adjuntará automáticamente al reporte</p>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-10 mt-6">
                                        ...
                                        <motion.button
                                            ...
                                            className="w-28 h-28 rounded-2xl bg-white text-red-600 flex items-center justify-center shadow-[0_20px_50px_rgba(0,0,0,0.4)] group"
                                        >
                                        ...
                                        <motion.button
                                            ...
                                            className="w-20 h-20 rounded-2xl bg-white/10 text-white border border-white/20 flex items-center justify-center hover:bg-white/20 transition-colors"
                                        >
                                            <X size={32} />
                                        </motion.button>
                                    </div>
                                ...
                                >
                                    ...
                                    {viewerData && (viewerData.plate || viewerData.name || viewerData.unit) && (
                                        <>
                                            ...
                                            >
                                                {/* Plate Number - Large and Prominent (tocar = Reporte de Inteligencia) */}
                                                {viewerData.plate && (
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); openPlateAnalysis(viewerData.plate); }}
                                                        className="block text-left bg-gradient-to-br from-white via-slate-100 to-slate-200 px-8 py-4 rounded-2xl border-4 border-black shadow-lg active:scale-95 transition-transform relative"
                                                    >
                                                        <div className="text-[10px] font-bold text-slate-600 uppercase tracking-[0.3em] mb-1 flex items-center gap-1.5">Matrícula Detectada {loadingAnalysis ? <Loader2 size={11} className="animate-spin" /> : <Activity size={11} className="text-[#B20D30]" />}</div>
                                                ...
                                                {viewerData.confidence !== undefined && (
                                                    <div className="bg-black/80 backdrop-blur-md px-4 py-2 rounded-xl border border-white/20">
                                                        <div className="text-[9px] font-bold text-white/60 uppercase tracking-wider mb-1">Confianza</div>
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-sm font-bold text-white">{viewerData.confidence}%</span>
                                                        </div>
                                                    </div>
                                                )}
                                            </motion.div>
                                            ...
                                            >
                                                {/* Owner Card */}
                                                {viewerData.name && (
                                                    <div className="bg-black/80 backdrop-blur-md px-6 py-4 rounded-2xl border border-white/20 shadow-lg">
                                                        <div className="flex items-center gap-3 mb-3">
                                                            ...
                                                            <div className="text-[10px] font-bold text-white/60 uppercase tracking-wider">Propietario</div>
                                                        </div>
                                                        <div className="text-xl font-bold text-white uppercase tracking-wide">{viewerData.name}</div>
                                                    </div>
                                                )}
                                                ...
                                                {viewerData.unit && (
                                                    <div className="bg-black/80 backdrop-blur-md px-6 py-4 rounded-2xl border border-white/20 shadow-lg">
                                                        <div className="flex items-center gap-3 mb-2">
                                                            ...
                                                            <div className="text-[10px] font-bold text-white/60 uppercase tracking-wider">Lote / Unidad</div>
                                                        </div>
                                                        <div className="text-lg font-bold text-white uppercase">{viewerData.unit}</div>
                                                    </div>
                                                )}
                                                ...
                                                {(viewerData.vehicleBrand || viewerData.vehicleModel) && (
                                                    <div className="bg-black/80 backdrop-blur-md px-6 py-4 rounded-2xl border border-white/20 shadow-lg">
                                                        <div className="flex items-center gap-3 mb-2">
                                                            ...
                                                            <div className="text-[10px] font-bold text-white/60 uppercase tracking-wider">Vehículo</div>
                                                        </div>
                                                        <div className="text-lg font-bold text-white uppercase">{`${viewerData.vehicleBrand || ""} ${viewerData.vehicleModel || ""}`.trim()}</div>
                                                    </div>
                                                )}
                                                ...
                                                {(viewerData.deviceName || viewerData.device?.name) && (
                                                    <div className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest mr-1">Cámara:</span>
                                            ...
                                            >
                                                <div className="bg-gradient-to-r from-black/90 via-black/80 to-black/90 backdrop-blur-xl px-8 py-5 rounded-2xl border border-white/20 shadow-lg">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-6">
                                                            <div>
                                                                <div className="text-[9px] font-bold text-white/50 uppercase tracking-wider mb-1">Análisis Rápido</div>
                                ...
                                {activeTab === 'control' && viewerData && (viewerData.plate || viewerData.name === "Desconocido" || !viewerData.name) && (
                                    ...
                                    >
                                        <button
                                            ...
                                            className="h-20 px-12 rounded-2xl bg-emerald-500/40 backdrop-blur-3xl border-2 border-emerald-500/30 text-white font-bold text-lg uppercase tracking-widest shadow-lg flex items-center gap-4 transition-all hover:bg-emerald-500/60 active:scale-95 disabled:opacity-50"
                                        >
                        ...
                        showNormalizationModal && (
                            ...
                            >
                                <motion.div
                                    ...
                                    animate={{ opacity: 1, scale: 1 }}
                                    exit={{ opacity: 0, scale: 0.9 }}
                                    className="bg-white rounded-3xl shadow-lg max-w-2xl w-full p-8"
                                    onClick={(e) => e.stopPropagation()}
                                >
                                    <div className="flex items-center gap-4 mb-6">
                                        ...
                                        <div>
                                            <h2 className="text-2xl font-bold uppercase tracking-tight text-black">Normalizar Alerta</h2>
                                    ...
                                    <div className="flex gap-3">
                                        <button
                                            ...
                                            className="flex-1 h-14 bg-slate-100 hover:bg-slate-200 rounded-xl font-bold uppercase tracking-wider text-sm text-black transition-all"
                                        >
                                            Cancelar
                                        </button>
                                        <button
                                            ...
                                            className="flex-1 h-14 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 disabled:cursor-not-allowed rounded-xl font-bold uppercase tracking-wider text-sm text-white transition-all shadow-lg shadow-emerald-600/20"
                                        >
                    ...
                    {showGuardList && (
                        ...
                        >
                            <motion.div
                                ...
                                animate={{ opacity: 1, scale: 1 }}
                                exit={{ opacity: 0, scale: 0.9 }}
                                className="bg-white rounded-3xl shadow-lg max-w-4xl w-full p-8 overflow-hidden flex flex-col max-h-[90vh]"
                                onClick={(e) => e.stopPropagation()}
                            >
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h2 className="text-3xl font-bold uppercase tracking-tighter text-black">Guardias Disponibles</h2>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 overflow-y-auto p-2">
                                        {guardsList.map((guard) => (
                                            ...
                                            >
                                                ...
                                                <div className="text-center">
                                                    <p className="text-lg font-bold uppercase text-black group-hover:text-[#B20D30] transition-colors line-clamp-1">{guard.name}</p>
                    ...
                    {showSettingsModal && (
                        ...
                        >
                            <motion.div
                                ...
                                animate={{ opacity: 1, scale: 1 }}
                                exit={{ opacity: 0, scale: 0.9 }}
                                className="bg-white rounded-3xl shadow-lg max-w-2xl w-full p-8 overflow-y-auto max-h-[90vh]"
                                onClick={(e) => e.stopPropagation()}
                            >
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div>
                                        <h2 className="text-2xl font-bold uppercase tracking-tight text-black">Configuración</h2>
                                        <p className="text-sm text-black/60 font-bold">Personalización de la interfaz</p>
                                    </div>
                                </div>

                                <div className="mb-8 p-6 bg-slate-50 rounded-2xl border border-slate-200">
                                    <h3 className="text-sm font-bold uppercase tracking-wider text-black mb-4">Permisos y Diagnóstico</h3>
                                    <button
                                        onClick={() => {
                                            if ("Notification" in window) Notification.requestPermission();
                                            if (!navigator.mediaDevices?.getUserMedia) { toast.error({ title: "Cámara/mic requieren HTTPS" }); return; }
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-4">
                                        <Label className="text-xs font-bold uppercase tracking-wider text-black/60">Logo Central (Sin bordes)</Label>
                                        <div className="flex gap-6 items-center bg-slate-50 p-6 rounded-2xl border-2 border-slate-100">
                                    ...
                                    <div className="space-y-4">
                                        <Label className="text-xs font-bold uppercase tracking-wider text-black/60">Color de Títulos de Tablas</Label>
                                        <div className="flex gap-4 flex-wrap">
                                            ...
                                            <div className="flex items-center gap-2 ml-4">
                                                ...
                                                />
                                                <span className="text-[10px] font-bold uppercase text-black/40">Personalizado</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="flex gap-4 mt-10">
                                    <button
                                        ...
                                        className="flex-1 h-16 bg-slate-100 hover:bg-slate-200 rounded-2xl font-bold uppercase tracking-widest text-xs text-black transition-all disabled:opacity-50"
                                    >
                                        Cancelar
                                    </button>
                                    <button
                                        ...
                                        className="flex-1 h-16 bg-black hover:bg-neutral-800 rounded-2xl font-bold uppercase tracking-widest text-xs text-white transition-all shadow-xl disabled:opacity-50 flex items-center justify-center gap-3"
                                    >
            ...
            <AnimatePresence>
                {showUpdateSplash && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[5000] bg-blue-600 flex flex-col items-center justify-center p-8 text-white overflow-hidden shadow-lg"
                    >
                        <motion.div
                            animate={{ rotate: 360 }}
                        ...
                        <h2 className="text-5xl font-bold uppercase tracking-tighter mb-4 text-center">
                            {isCheckingUpdate ? "Verificando Versión" : (updateAvailable ? "Actualización Lista" : "Omniaccess Pro")}
                        </h2>
                        <p className="text-xs font-bold text-white/50 uppercase tracking-[0.4em] mb-12 text-center">
                        ...
                        {updateAvailable && (
                            <div className="w-full max-w-md space-y-4">
                                <button
                                    ...
                                    className="w-full h-20 bg-white text-blue-600 rounded-2xl font-bold uppercase tracking-widest text-sm active:scale-95 transition-all shadow-lg flex items-center justify-center gap-3"
                                >
                                    <Download size={24} /> Actualizar ahora
                                </button>
                                <button
                                    onClick={() => setShowUpdateSplash(false)}
                                    className="w-full h-20 bg-blue-700 text-white rounded-2xl font-bold uppercase tracking-widest text-sm active:scale-95 transition-all"
                                >
                ...
                {showSearchFillModal && recentRecords.length > 0 && (
                    ...
                    >
                        <motion.div
                            ...
                            animate={{ scale: 1, y: 0 }}
                            exit={{ scale: 0.9, y: 20 }}
                            className="w-full max-w-3xl bg-white rounded-2xl p-12 shadow-lg space-y-10 relative overflow-hidden"
                            onClick={(e) => e.stopPropagation()}
                        >
                            ...
                            <div className="space-y-3 text-center relative">
                                ...
                                <h3 className="text-4xl font-bold uppercase tracking-tighter text-slate-900 leading-none">Registros Históricos</h3>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.3em]">Encontramos datos previos para esta matrícula</p>
                            </div>
                            ...
                            <div className="space-y-4 max-h-[45vh] overflow-y-auto pr-4 custom-scrollbar">
                                {recentRecords.map((record) => (
                                    <button
                                        ...
                                        className="w-full p-8 rounded-2xl bg-slate-50 border-2 border-slate-100 flex items-center justify-between text-left active:scale-[0.98] transition-all group hover:bg-white hover:border-blue-400 hover:shadow-lg hover:shadow-blue-600/10"
                                    >
                                        <div className="space-y-3">
                                            ...
                                            <p className="text-2xl font-bold text-slate-900 uppercase tracking-tight group-hover:text-blue-600 transition-colors">{record.name || "Sin nombre"}</p>
                            ...
                            <div className="flex gap-4 pt-4">
                                <button
                                    onClick={() => setShowSearchFillModal(false)}
                                    className="flex-1 h-20 bg-slate-100 text-slate-400 rounded-3xl font-bold uppercase tracking-widest text-xs hover:bg-slate-200 transition-all"
                                >
            ...
            <AnimatePresence>
                {plateAnalysis && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[3100] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-6"
                    ...
                    >
                        <motion.div
                            ...
                            className="w-full max-w-2xl bg-white rounded-3xl p-5 md:p-7 space-y-5 shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar"
                            onClick={(e) => e.stopPropagation()}
                        >
                            <div className="absolute top-0 left-0 right-0 h-24 bg-gradient-to-b from-[#B20D30]/5 to-transparent pointer-events-none rounded-t-3xl" />

                            <div className="flex justify-between items-center gap-3 relative">
                                <div className="min-w-0">
                                    <h3 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900 leading-none uppercase truncate">Reporte de Inteligencia</h3>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-1">Análisis de comportamiento LPR</p>
                                </div>
                                <div className="bg-[#B20D30] px-4 py-2.5 rounded-2xl shadow-lg shadow-[#B20D30]/20 shrink-0">
                                    <span className="text-white font-black text-xl md:text-2xl tracking-[0.15em]">{plateAnalysis.events[0]?.plateDetected || (viewerData as any)?.plate}</span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center justify-between">
                                    <div>
                                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Frecuencia</p>
                                        <p className="text-3xl font-black text-[#B20D30] leading-none">{plateAnalysis.totalEvents}</p>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-[9px] font-bold text-slate-900 uppercase">Pasadas</div>
                                        <div className="text-[8px] font-bold text-slate-400">Últimos 7 días</div>
                                    </div>
                                </div>
                            </div>

                            <div className="space-y-2.5">
                                <h4 className="text-[10px] font-bold text-slate-900 uppercase tracking-[0.35em] ml-1">Cronología de eventos</h4>
                                <div className="space-y-2">
                                    {plateAnalysis.events.map((ev: any, i: number) => (
                                        <div key={ev.id} className="flex items-center justify-between gap-2 p-3 rounded-2xl bg-slate-50 border border-slate-100">
                                            <div className="flex items-center gap-3 min-w-0">
                                                <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center shadow-sm shrink-0", ev.direction === 'ENTRY' ? "bg-emerald-100 text-emerald-600" : "bg-orange-100 text-orange-600")}>
                                                    {ev.direction === 'ENTRY' ? <LogIn size={18} /> : <LogOut size={18} />}
                                                </div>
                                    ...
                                    {(!plateAnalysis.events || plateAnalysis.events.length === 0) && (
                                        <p className="text-xs font-semibold text-slate-400 text-center py-8">Sin eventos previos para esta matrícula</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <button
                                onClick={() => setPlateAnalysis(null)}
                                className="w-full h-14 bg-slate-900 text-white rounded-2xl font-bold uppercase tracking-[0.2em] text-sm active:scale-95 transition-all shadow-lg sticky bottom-0"
                            >
                                Cerrar
                            </button>
                        </motion.div>
...
function DetailCard({ label, value, icon }: any) {
    return (
        <div className="flex items-center gap-6 p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100 group hover:bg-white hover:shadow-xl transition-all duration-300">
            ...
            <div>
                <p className="text-[10px] font-bold uppercase text-slate-400 tracking-[0.2em] mb-1">{label}</p>
                <p className="text-xl font-bold uppercase text-slate-900 tracking-tight">{value || "Sin especificar"}</p>
            </div>
        </div>
    );
}
...
function BottomTab({ icon, active, onClick, label, small, alertActive }: any) {
    return (
        <motion.button
            ...
            className={cn(
                small ? "w-20 h-14 md:w-24 md:h-16" : "w-16 h-16 md:w-24 md:h-20",
                "rounded-[1rem] md:rounded-2xl flex flex-col items-center justify-center gap-1 md:gap-1.5 transition-all relative overflow-hidden shrink-0",
                active
        ...
        >
            ...
            <span className={cn(
                ...
                "font-bold uppercase tracking-widest relative z-10 transition-all duration-300",
            ...
            {active && (
                <motion.div
                    ...
                    className="absolute inset-0 bg-gradient-to-br from-[#B20D30]/10 to-[#B20D30]/5 rounded-xl border-2 border-[#B20D30]/20 shadow-lg"
...
function OriginButton({ active, onClick, icon, label, activeClass }: any) {
    return (
        <motion.button
            ...
            className={cn(
                "h-20 rounded-xl border-2 flex flex-col items-center justify-center gap-1.5 transition-all duration-300 relative overflow-hidden shadow-sm",
...
function AppField({ label, icon, value, onChange, placeholder }: any) {
    return (
        <div className="space-y-2 md:space-y-4">
            <Label className="text-[10px] md:text-xs font-bold uppercase text-black tracking-[0.2em] md:tracking-[0.3em] flex items-center gap-2 md:gap-3 ml-1 md:ml-2">
            ...
            <Input
                ...
                className="h-14 md:h-20 bg-white border border-black/20 focus:ring-4 focus:ring-[#B20D30]/20 rounded-xl md:rounded-2xl text-black font-bold text-sm md:text-lg tracking-wide px-6 md:px-10 placeholder:text-black/20 transition-all shadow-sm"
            />
        </div>
    );
}
...
function SensorStatus({ label, active }: { label: string, active: boolean }) {
    return (
        <div className="flex items-center justify-between group cursor-help">
            <span className="text-[11px] font-bold uppercase tracking-tight text-black/40 group-hover:text-black transition-colors">{label}</span>
...
function HistoryItem({ entry, onDelete }: any) {
    return (
        <motion.div whileHover={{ y: -8 }} className="bg-white border-2 border-slate-100 rounded-2xl overflow-hidden group shadow-sm hover:shadow-xl hover:border-[#B20D30]/20 transition-all duration-500">
            <div className="aspect-[4/3] relative overflow-hidden bg-slate-50 border-b-2 border-slate-100">
                ...
                <div className={cn("absolute top-6 left-6 px-3 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-[0.2em] shadow-lg backdrop-blur-md", entry.type === "ENTRY" ? "bg-[#B20D30] text-white" : "bg-orange-500 text-white")}>
                    {entry.type === "ENTRY" ? "Ingreso" : "Egreso"}
                </div>
            </div>
            <div className="p-8">
                <div className="flex justify-between items-center mb-6">
                    <h4 className="text-2xl font-bold text-black tracking-tighter uppercase">{entry.plate || "S/ Placa"}</h4>
                    <div className="flex items-center gap-2">
                        {entry.audioPath && (
                            <button onClick={() => {
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                    <div>
                        <p className="text-[10px] font-bold text-black/40 uppercase tracking-widest leading-none mb-2">Sujeto</p>
                        <p className="text-xs font-bold text-black uppercase truncate">{entry.name || "Identidad Reservada"}</p>
                    </div>
                    <div>
                        <p className="text-[10px] font-bold text-black/40 uppercase tracking-widest leading-none mb-2">Destino</p>
                        <p className="text-xs font-bold text-[#B20D30] uppercase truncate">{entry.destination || "Sin Destino"}</p>
                    </div>
                    {entry.latitude && (
                        <div className="col-span-2 mt-2 pt-4 border-t-2 border-slate-50 flex items-center gap-2 text-[9px] text-black/30 font-bold uppercase tracking-widest">
...
function RollingCharacter({ char, isFocused }: { char: string, isFocused: boolean }) {
    return (
        <div className={cn(
            ...
            <span className={cn(
                "text-xl sm:text-4xl md:text-6xl font-bold tracking-tighter",
...
function TactilePlateInput({ value, onChange, onCameraClick, playTactileSound }: { value: string, onChange: (v: string) => void, onCameraClick?: () => void, playTactileSound?: () => void }) {
    ...
    return (
        <div className="w-full flex flex-col gap-4 items-center relative">
            ...
            >
                <p className="text-[10px] font-bold text-[#B20D30] uppercase tracking-[0.4em] animate-pulse">Escribe la matricula en cada casillero</p>
            </motion.div>
        </div>
    );
}
...
function DeviceEntry({ name, status }: { name: string, status: string }) {
    return (
        <div className="flex items-center gap-4 px-2 py-1 group cursor-default">
            ...
            <span className="text-[10px] font-bold text-black font-bold uppercase tracking-widest group-hover:text-black transition-colors">{name}</span>
        </div>
    );
}
...
function OriginPickerButton({ active, onClick, icon, label, sub }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string, sub: string }) {
    return (
        <motion.button
            ...
            onClick={onClick}
            className={cn(
                "h-56 rounded-2xl border-2 flex flex-col items-center justify-center gap-4 transition-all relative overflow-hidden group shadow-xl",
                active
        ...
        >
            ...
            <div className="text-center">
                <p className="text-2xl font-bold uppercase tracking-tighter">{label}</p>
                <p className={cn("text-[9px] font-bold uppercase tracking-widest mt-1", active ? "text-white/40" : "text-black/30")}>{sub}</p>
            </div>
            {active && (
...
function NotificationOverlay({ type, title, message, onClose }: { type: string, title: string, message: string, onClose: () => void }) {
    ...
    return (
        ...
        >
            ...
            >
                <div className="w-32 h-32 rounded-2xl flex items-center justify-center mb-10 shadow-lg relative bg-white text-[#B20D30]">
                ...
                <p className="text-xl font-bold text-white/60 uppercase tracking-widest leading-relaxed">
                    {message}
                </p>
