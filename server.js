...
const handleWebhook = async (req, res, logPrefix) => {
    try {
        ...
        // --- Bounding box de la patente (para overlay #28). Hikvision: normalmente 0-1000 ---
        const _findRect = (o) => {
            if (!o || typeof o !== 'object') return null;
            const bb = o.boundingBox || o.BoundingBox || o.plateRect || o.PlateRect || o.rect || o.Rect || o.region;
            if (!bb || typeof bb !== 'object') return null;
            const x = parseFloat(bb.x ?? bb.X ?? bb.positionX ?? bb.left ?? bb.minX);
            const y = parseFloat(bb.y ?? bb.Y ?? bb.positionY ?? bb.top ?? bb.minY);
            const w = parseFloat(bb.width ?? bb.Width ?? bb.w);
            const h = parseFloat(bb.height ?? bb.Height ?? bb.h);
            if ([x, y, w, h].every((n) => !isNaN(n))) return { x, y, w, h };
            return null;
        };
        const _picInfo = eventAlert.ANPR?.pictureInfoList?.pictureInfo || xmlData.ANPR?.pictureInfoList?.pictureInfo;
        const _picArr = Array.isArray(_picInfo) ? _picInfo : (_picInfo ? [_picInfo] : []);
        ...
        try { console.log(`${logPrefix} [ANPR-RAW]`, JSON.stringify(eventAlert.ANPR || xmlData.ANPR || {}).slice(0, 1900)); } catch (e) {}
        ...
        // Timestamp: trust the camera clock ONLY if it's within 10 min of server time.
        // Some cameras have drifted/unsynced clocks (seen 11h off) which would poison
        // "hace X" and event ordering. Otherwise fall back to server ingest time.
        let eventTimestamp = new Date();
        const cameraDateTime = xmlData.dateTime || eventAlert.dateTime;
        if (cameraDateTime) {
            try {
                const camT = new Date(cameraDateTime);
                if (!isNaN(camT.getTime()) && Math.abs(Date.now() - camT.getTime()) <= 10 * 60 * 1000) {
                    eventTimestamp = camT;
                }
            } catch (e) { /* keep server time */ }
        }
        ...
        // Recorte de patente (licensePlatePicture) + guardarlo aparte
        let plateCropPath = "";
        try {
            const cropImg = images.find(i => /plate|licen|patent/i.test(i.name || "")) || (images.length > 1 ? images[images.length - 1] : null);
            if (cropImg && cropImg.buffer && cropImg !== imageFile && cropImg.size > 300) {
                const cfn = `plate_${finalPlate}_${eventTimestamp.getTime()}.jpg`;
                plateCropPath = await uploadToS3(cropImg.buffer, cfn, cropImg.mimeType || "image/jpeg", "lpr");
                console.log(`${logPrefix} [S3] Plate crop uploaded: ${plateCropPath}`);
            }
        } catch (e) { console.error(`${logPrefix} [S3] Plate crop upload FAILED: ${e.message}`); }
        ...
        // Fallback: si el webhook ANPR llego SIN imagen, capturar snapshot por ISAPI (ej. P6-202)
        if (!relativeImagePath && device) {
            try {
                const snapBuf = await fetchCameraSnapshot(device);
                if (snapBuf && snapBuf.length > 500) {
                    const fnSnap = `hik_${finalPlate}_${eventTimestamp.getTime()}.jpg`;
                    relativeImagePath = await uploadToS3(snapBuf, fnSnap, "image/jpeg", "lpr");
                    console.log(`${logPrefix} [S3] Snapshot fallback (ISAPI) SUCCESS: ${relativeImagePath}`);
                }
            } catch (e) { console.error(`${logPrefix} [S3] Snapshot fallback FAILED: ${e.message}`); }
        }
...
// ── Guard GPS relay ─────────────────────────────────────────────────────
// Las tablets PWA (/guard) emiten guard_location_update (watchPosition) y
// guard_presence (heartbeat). Acá se agregan, se expiran y se re-emiten como
// guard_locations a TODOS los clientes (consola de guardia, /admin/mapa, etc.)
const guardState = new Map(); // socket.id -> { guardName, lat, lng, accuracy, ts, guardPhoto, deviceInfo, socketId }
function guardsArray() {
    const now = Date.now();
    const out = [];
...
// resolve_backup / cancel_backup y escucha alert_status / backup_* / active_missions.
// Este relay se perdió al portar el módulo (igual que el GPS) -> se re-agrega.
const backupMissions = new Map(); // id -> mission
let panicState = { active: false, triggeredBy: null, explanation: "", timestamp: null };
...
io.on("connection", (socket) => {
    ...
    // Estado actual de pánico + misiones al recién conectado
    try {
        socket.emit("alert_status", panicState);
        socket.emit("active_missions", Array.from(backupMissions.values()));
    } catch (e) {}

    // ── Pánico ──
    socket.on("alert_toggle", (data) => {
        try {
            panicState = {
                active: !!(data && data.active),
                triggeredBy: (data && data.triggeredBy) || "Guardia",
                explanation: (data && data.explanation) || "",
                timestamp: new Date().toISOString()
            };
            io.emit("alert_status", panicState);
        } catch (e) { console.error("[relay alert_toggle]", e && e.message); }
    });

    // ── Backup / misiones ──
    socket.on("request_backup", (mission) => {
        try {
            if (!mission || !mission.id) return;
            const m = { ...mission, requesterSocketId: socket.id, status: mission.status || "PENDING", ts: Date.now() };
            backupMissions.set(mission.id, m);
            socket.broadcast.emit("backup_requested", m); // a los OTROS guardias (no al que lo pidió)
            io.emit("active_missions", Array.from(backupMissions.values()));
        } catch (e) { console.error("[relay request_backup]", e && e.message); }
    });

    socket.on("respond_backup", (data) => {
        try {
            if (!data || !data.requestId) return;
            const m = backupMissions.get(data.requestId);
            if (m) { m.status = data.accepted ? "ACCEPTED" : "REJECTED"; m.responderId = socket.id; m.responderName = data.responderName || "Guardia"; backupMissions.set(data.requestId, m); }
            io.emit("backup_status_update", { requestId: data.requestId, accepted: !!data.accepted, responderId: socket.id, responderName: (data && data.responderName) || "Guardia" });
            io.emit("active_missions", Array.from(backupMissions.values()));
        } catch (e) { console.error("[relay respond_backup]", e && e.message); }
    });

    socket.on("resolve_backup", (data) => {
        try {
            if (!data || !data.requestId) return;
            backupMissions.delete(data.requestId);
            io.emit("backup_resolved", { requestId: data.requestId, resolverName: (data && data.guardName) || "Guardia", outcome: (data && data.outcome) || "", notes: (data && data.notes) || "" });
            io.emit("active_missions", Array.from(backupMissions.values()));
        } catch (e) { console.error("[relay resolve_backup]", e && e.message); }
    });
    ...
    socket.on("cancel_backup", (data) => {
        try {
            if (!data || !data.requestId) return;
            backupMissions.delete(data.requestId);
            io.emit("backup_cancelled", { requestId: data.requestId, cancelledBy: (data && data.guardName) || "Guardia", reason: (data && data.reason) || "" });
            io.emit("active_missions", Array.from(backupMissions.values()));
        } catch (e) { console.error("[relay cancel_backup]", e && e.message); }
    });
    ...
    socket.on("guard_location_update", (data) => {
        try {
            ...
            guardState.set(socket.id, { ...prev, guardName: data.guardName || prev.guardName || "Guardia", lat: data.lat, lng: data.lng, accuracy: data.accuracy || null, ts: Date.now() });
            broadcastGuards();
        } catch (e) { }
    });
    ...
    // ── Telemetría nativa (batería/señal/brújula/pasos) desde la APK ──
    socket.on("guard_telemetry", (data) => {
        try {
            if (!data) return;
            const prev = guardState.get(socket.id) || { guardName: data.guardName || "Guardia", lat: null, lng: null, ts: Date.now() };
            guardState.set(socket.id, { ...prev,
                guardName: data.guardName || prev.guardName,
                battery: (data.battery != null ? data.battery : prev.battery),
                signal: (data.signal != null ? data.signal : prev.signal),
                heading: (data.heading != null ? data.heading : prev.heading),
    ...
    // ── Rondas: fichaje y punto vencido (passthrough para supervisores) ──
    socket.on("guard_checkpoint", (data) => { try { io.emit("guard_checkpoint", { ...(data || {}), ts: Date.now() }); } catch (e) {} });
    socket.on("guard_checkpoint_missed", (data) => { try { io.emit("guard_checkpoint_missed", { ...(data || {}), ts: Date.now() }); } catch (e) {} });

    // ── Geocerca: guardia fuera de zona ──
    socket.on("guard_fence", (data) => {
    ...
    try {
        ...
        for (const d of devs) {
            ...
            await new Promise((resolve) => {
                let done = false;
                const sock = _netReach.connect({ host, port: 554, timeout: 3000 }, () => {
                    done = true; try { sock.destroy(); } catch (e) {}
                    prisma.device.update({ where: { id: d.id }, data: { lastOnlinePull: new Date() } }).catch(() => {});
                    resolve();
                });
                sock.on("error", () => { if (!done) { try { sock.destroy(); } catch (e) {} resolve(); } });
...
// ===================== MERODEO (loitering) detection =====================
async function getMerodeoConfig() {
    const now = Date.now();
    if (global.__merodeoCfg && (now - (global.__merodeoCfgAt || 0)) < 30000) return global.__merodeoCfg;
    let m = {};
    try {
        const rows = await prisma.setting.findMany({ where: { key: { in: ['MERODEO_ENABLED','MERODEO_WINDOW_MIN','MERODEO_THRESHOLD','MERODEO_COOLDOWN_MIN','MERODEO_EXCLUDE_RESIDENTS'] } } });
        rows.forEach(r => { m[r.key] = r.value; });
    } catch (e) {}
    const cfg = {
        enabled: (m.MERODEO_ENABLED == null ? 'true' : m.MERODEO_ENABLED) === 'true',
        windowMin: parseInt(m.MERODEO_WINDOW_MIN || '30') || 30,
        threshold: parseInt(m.MERODEO_THRESHOLD || '3') || 3,
        cooldownMin: parseInt(m.MERODEO_COOLDOWN_MIN || '30') || 30,
        excludeResidents: (m.MERODEO_EXCLUDE_RESIDENTS == null ? 'true' : m.MERODEO_EXCLUDE_RESIDENTS) === 'true',
    };
...
async function checkMerodeo(plate, device, event, logPrefix) {
    ...
    if (!clean || ['NO_LEIDA','UNKNOWN','S/P','DOOR_OPEN','DOOR_CLOSE'].includes(clean)) return;
    const cfg = await getMerodeoConfig();
    if (!cfg.enabled) return;
    // Skip authorized residents (GRANT with a linked user), if configured
    if (cfg.excludeResidents && event.decision === 'GRANT' && event.userId) return;
    global.__merodeoCooldown = global.__merodeoCooldown || new Map();
    const now = Date.now();
    const last = global.__merodeoCooldown.get(clean);
    if (last && (now - last) < cfg.cooldownMin * 60000) return;
    const since = new Date(now - cfg.windowMin * 60000);
    const passes = await prisma.accessEvent.findMany({
        where: { accessType: 'PLATE', plateDetected: clean, timestamp: { gte: since } },
        select: { direction: true, timestamp: true, deviceId: true },
        orderBy: { timestamp: 'asc' }
    });
    const count = passes.length;
    if (count < cfg.threshold) return;
    const distinctDevices = new Set(passes.map(p => p.deviceId).filter(Boolean)).size;
    global.__merodeoCooldown.set(clean, now);
    console.warn(`${logPrefix} [MERODEO] ${clean}: ${count} pasadas en ${cfg.windowMin}min (${distinctDevices} acceso/s)`);
    try {
        await prisma.bitacora.create({ data: {
            type: 'MERODEO',
            plate: clean,
            photoPath: event.snapshotPath || null,
            notes: `Merodeo: ${count} pasadas en ${cfg.windowMin} min a traves de ${distinctDevices} acceso/s. Ultima camara: ${(device && device.name) || 'N/D'}.`,
            accessEventId: event.id,
        }});
    } catch (e) { /* accessEventId unique or bitacora issue - ignore */ }
    if (global.io) {
        global.io.emit('merodeo_alert', {
            plate: clean, count, windowMin: cfg.windowMin, distinctDevices,
